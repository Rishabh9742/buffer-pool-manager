...
class LRUReplacer(Replacer):
    ...
    def unpin(self, page_id):
        logging.info(f"Page {page_id} is unpinned. It is added to the free_frames list")
        if page_id not in self.free_frames:
            self.free_frames.append(page_id)

    def victim(self):
        if not self.free_frames:
            logging.warning("No free frames available for eviction.")
            return None
        page_to_evict = self.free_frames.pop(0)
        logging.info(f"Evicting page {page_to_evict}. Removed from the free frames list.")
        return page_to_evict

    def replacerSize(self):
        logging.info(f"Number of free frames: {len(self.free_frames)}")
